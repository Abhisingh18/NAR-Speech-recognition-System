"""
PROPOSED architecture schematic (Pillow renderer) for the filler_asr SA model
with two planned changes vs model_sa.py:
  (1) sinusoidal positional encoding ADDED to the HuBERT tap before the 2 new
      self-attention layers;
  (2) a duration-based frame budget at the loss: keep the first
      n_keep = ceil(duration_seconds * 25) frames, mask the rest out of the
      framewise cross-entropy.
Output: proposed_arch.png
"""
...
C_NEW    = (255, 233, 204); C_NEWBRD = (210, 130, 40)   # new / proposed
C_MASK   = (226, 216, 240); C_MASKBRD = (120, 90, 170)  # loss masking
...
# ---------- title ----------
ctext(CX, 38, "filler_asr  —  PROPOSED Self-Attention model (xlarge)", ft)
ctext(CX, 68, "model_sa.py  +  (1) sinusoidal pos-enc   (2) duration frame-budget loss mask", f(FM, 15), C_GREY)
...
ctext(CX, iy+15, "Conv feature extractor  (7 conv layers, stride /320)", fr)
...
y = fb_top + 300
varrow(CX, y, y+38); y += 38
ctext(CX-470, y-19, "hidden_states", fm, C_GREY, anchor="lm")

# =================== (1) NEW: sinusoidal positional encoding ===================
box(CX-390, y, CX+390, y+66, C_NEW, C_NEWBRD, w=3)
ctext(CX, y+21, "(1)  + Sinusoidal Positional Encoding   [NEW]", fr, (170, 95, 20))
ctext(CX, y+43, "h = hidden_states + PE(t)   ·   PE fixed (non-learned)   ·   (B, T, 1280)", fm, C_GREY)
y += 66
varrow(CX, y, y+32); y += 32
...
ctext(CX, y+38, "per-frame class scores", fs, C_GREY)
y += 54
varrow(CX, y, y+34); y += 34

# =================== (2) NEW: duration frame-budget loss mask ===================
box(CX-450, y, CX+450, y+96, C_MASK, C_MASKBRD, w=3)
ctext(CX, y+22, "(2)  Duration frame-budget mask   [NEW]", fr, (95, 60, 150))
ctext(CX, y+48, "n_keep = ceil(audio_duration_seconds * 25)", fm, (60, 40, 110))
ctext(CX, y+72, "keep logits[:, :n_keep]  ·  mask the remaining frames out of the loss", fs, C_GREY)
y += 96
varrow(CX, y, y+32); y += 32

# ---------- loss ----------
box(CX-330, y, CX+330, y+58, C_IO, C_IOBRD)
ctext(CX, y+18, "Framewise Cross-Entropy", fr)
ctext(CX, y+39, "computed over the first n_keep frames only", fs, C_GREY)
...
chip(30,  C_FROZEN, C_FRBRD, "frozen")
chip(150, C_TRAIN,  C_TRBRD, "trainable")
chip(290, C_NEW,    C_NEWBRD,"new / proposed")
chip(470, C_ADD,    C_ADDBRD,"add & norm")
chip(620, C_MASK,   C_MASKBRD,"loss mask")
dr.line([(770, ly+9),(806, ly+9)], fill=C_RES, width=2)
dr.text((814, ly+9), "residual", font=fs, fill=C_RES, anchor="lm")

img.save("proposed_arch.png")
print("wrote proposed_arch.png", img.size)
