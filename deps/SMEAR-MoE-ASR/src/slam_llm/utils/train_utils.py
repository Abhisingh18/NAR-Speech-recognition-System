...
def train(model, train_dataloader,eval_dataloader, tokenizer, optimizer, lr_scheduler, gradient_accumulation_steps, train_config, log_config, fsdp_config=None, local_rank=None, rank=None):
    ...
    # use_bf16: bf16 autocast, no GradScaler needed. ~2.6x faster than fp32 here and
    # numerically safer than fp16 for gemma-3 (trained in bf16). use_fp16 keeps the
    # original fp16-autocast + GradScaler path. Neither set => full fp32.
    if getattr(train_config, "use_bf16", False):
    ...
    for epoch in range(train_config.num_epochs):
        ...
        with MemoryTrace() as memtrace:  # track the memory usage
            ...
            using_lr_of_optimizer = 0 # 0 MEANS lr_schefuler's and 1 means optimizer's
            # Can we select specific batch??
            for step, batch in enumerate(train_dataloader):
                ...
                with autocast():
                    # for tmpo in batch.keys():

                    #     print("--------------------------------------------",batch[f"{tmpo}"].shape,"_------------------------------")
                    # print(f"------------------------------------------{model}------------------------------")
                    outputs, *rest = model(**batch)
                acc = rest[0] if rest else -1

                if len(rest) > 1:
                    llm_loss = outputs.loss.clone()
                    mse_loss = rest[1].clone()
                    loss = outputs.loss + rest[1] 
                else:
                    loss = outputs.loss
                ...
                if log_config.use_wandb and step % log_config.log_interval == 0:
                    if train_config.enable_fsdp or train_config.enable_ddp:
                        if rank==0:
                            if len(rest) > 1:
                                wandb.log({"train_inner/train_inner_loss":loss, "train_inner/llm":llm_loss, "train_inner/load_balancing":mse_loss,  "train_inner/train_inner_accuracy":acc} , step=(epoch * total_length + step))
                            else:
                    ...
                    # if len(rest) > 2:
                        # print(f"router_prob_mean::::::: {rest[2]} at step {step}")
                    # print("HERE::::::::::::::::::",step, torch.cuda.memory_allocated() / 1024**2)
                ...
                if train_config.use_fp16:
                    ...
                    if (step + 1) % gradient_accumulation_steps == 0 or step == len(train_dataloader) - 1:
                        ...
                        if lr_scheduler is not None:
                            ...
                            using_lr_of_optimizer = 0
                        else:
                            current_lr = optimizer.param_groups[0]["lr"]
                            using_lr_of_optimizer = 1
                        if current_lr == 0:
                            if using_lr_of_optimizer == 0:
                                print(f"---------BREAKING AS CURRENT LR => {current_lr} OF EPOCH {epoch} AND SCHEDULER {lr_scheduler}, {lr_scheduler.get_last_lr()}  IS BECOMING ZERO scaler.scale(loss).backward()-------------------")
                                logger.info(f"---------BREAKING AS CURRENT LR => {current_lr} OF EPOCH {epoch} AND SCHEDULER {lr_scheduler}, {lr_scheduler.get_last_lr()}  IS BECOMING ZERO scaler.scale(loss).backward()-------------------")
                            else:
                                print(f"---------BREAKING AS CURRENT LR => {current_lr} OF EPOCH {epoch} AND OPTIMIZER {optimizer}, {optimizer.param_groups[0]}  IS BECOMING ZERO scaler.scale(loss).backward()--------------------")
                                logger.info(f"---------BREAKING AS CURRENT LR => {current_lr} OF EPOCH {epoch} AND OPTIMIZER {optimizer}, {optimizer.param_groups[0]}  IS BECOMING ZERO scaler.scale(loss).backward()--------------------")
                            break
                        if log_config.use_wandb and step % log_config.log_interval == 0:
                            if train_config.enable_fsdp or train_config.enable_ddp:
                                if rank==0:
                    ...
                    if (step + 1) % gradient_accumulation_steps == 0 or step == len(train_dataloader) - 1:
                        ...
                        if current_lr == 0:
                            if using_lr_of_optimizer == 0:
                                print(f"---------BREAKING AS CURRENT LR => {current_lr} OF EPOCH {epoch} AND SCHEDULER {lr_scheduler}, {lr_scheduler.get_last_lr()}  IS BECOMING ZERO loss.backward()-------------------")
                                logger.info(f"---------BREAKING AS CURRENT LR => {current_lr} OF EPOCH {epoch} AND SCHEDULER {lr_scheduler}, {lr_scheduler.get_last_lr()}  IS BECOMING ZERO loss.backward()-------------------")
                            else:
                                print(f"---------BREAKING AS CURRENT LR => {current_lr} OF EPOCH {epoch} AND OPTIMIZER {optimizer}, {optimizer.param_groups[0]}  IS BECOMING ZERO loss.backward()--------------------")
                                logger.info(f"---------BREAKING AS CURRENT LR => {current_lr} OF EPOCH {epoch} AND OPTIMIZER {optimizer}, {optimizer.param_groups[0]}  IS BECOMING ZERO loss.backward()--------------------")
                            break
                        if log_config.use_wandb and step % log_config.log_interval == 0:
                            if train_config.enable_fsdp or train_config.enable_ddp:
                                if rank==0:
                ...
                if (epoch * total_length + step + 1) % train_config.validation_interval == 0 and train_config.run_validation:
                    ...
                    if train_config.save_model:
                        if train_config.save_on_best_val_loss and (eval_epoch_loss < best_val_loss) or not train_config.save_on_best_val_loss: 
    ...
    if train_config.run_validation:
        # guard: validation may never have fired (e.g. total steps < validation_interval)
...
def evaluation(model,train_config, eval_dataloader, local_rank, tokenizer):
    ...
    if getattr(train_config, "use_bf16", False):
        ...
        autocast = nullcontext # (Fix:MZY): fix expected scalar type mismatch in norm 
