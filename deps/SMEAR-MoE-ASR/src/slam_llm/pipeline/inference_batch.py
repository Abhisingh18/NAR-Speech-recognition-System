...
def main(kwargs: DictConfig):
	...
	dataset_test = get_preprocessed_dataset(
        ...
        dataset_config,
        split="test",
		num_lang=kwargs.get('num_lang', 1)
    )
	...
	with open(pred_path, "w") as pred, open(gt_path, "w") as gt:
		for step, batch in tqdm(enumerate(test_dataloader), total=len(test_dataloader)):
			...
			# print(f"--------------------------Printing model.generate input--------------------------")
			...
			# batch_duration = sum(batch["durations"])
			...
			print(output_text)
			wer=jiwer.wer(batch["targets"][0], output_text[0].replace("\n", " "))
			cer=jiwer.cer(batch["targets"][0], output_text[0].replace("\n", " "))
		...
		if total_audio_duration > 0:
			...
			print(f"Average RTF: {rtf:.4f}")
		else:
			print(f"Total inference time: {total_inference_time:.1f}s (RTF n/a: durations not collected)")
