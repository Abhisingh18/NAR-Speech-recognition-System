...
# NOTE: get_{linear,cosine}_schedule_with_warmup live in transformers.optimization, which imports
# transformers.trainer_utils -> peft. This env's peft is older than transformers 5.5 expects, so
# that import chain breaks. Our recipe uses the exponential schedule (local LambdaLR), so we import
# those two lazily inside the non-exponential branch only (see build of `sched`).
...
# --- indic-nar-filler_asr: data2vec-aqc encoder swap + Hindi Devanagari tokenizer/normalizer ---
#     (these override the HuBERT model + English tokenizer/collator used above)
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from model_d2v_acmlm import build_d2v_acmlm, build_collator, D2V_CKPT
from text_norm_hi import build_acmlm_tokenizer_hi, normalize_hi
from text_norm_kn import normalize_kn                                   # Kannada + English (bilingual)
NORMALIZERS = {"hi": normalize_hi, "kn": normalize_kn}                  # --lang -> target normalizer
from data_tapcached_d2v import (TapCacheDataset, DataCollatorACMLMTapCached,  # --tap_cache: train from
                                LengthBucketedDistributedBatchSampler)       #  precomputed data2vec taps (1024-d, Hindi)
...
def main():
    ...
    # indic-nar: data2vec-aqc .pt (SSL default; a CTC-finetuned wav2vec_ctc also works -> CTC head dropped)
    ap.add_argument("--model_checkpoint", default=D2V_CKPT)
    ap.add_argument("--encoder_tgt_layer", type=int, default=None,
                    help="data2vec early-exit layer (0-indexed, fairseq-native); None = final layer")
    # defaults = this (abhishek's) copy, Kannada+English; nothing here points into another user's dirs
    _here = os.path.dirname(os.path.abspath(__file__))
    ap.add_argument("--vocab_dir", default=os.path.join(_here, "vocab_kn"))
    ap.add_argument("--train_manifest", default=os.path.join(_here, "data_kn", "train.jsonl"))
    ap.add_argument("--dev_manifest", default=os.path.join(_here, "data_kn", "dev.jsonl"))
    ap.add_argument("--out_dir", default=os.path.join(_here, "runs", "kn_en_d2vaqcKN_sa8_ACMLM_alibi_fps50_fw003_exp"))
    ...
    ap.add_argument("--lr_schedule", choices=["linear", "cosine", "exponential", "constant"], default="linear",
    ...
    ap.add_argument("--lang", choices=sorted(NORMALIZERS), default="hi",
                    help="target text normalizer: hi = normalize_hi (Devanagari only), "
                         "kn = normalize_kn (Kannada + lowercase Latin, bilingual). Must match --vocab_dir.")
    ...
    # indic-nar: Hindi Devanagari tokenizer (NFC + punct-stripped vocab_hi/vocab.json)
    ...
    if args.acmlm and args.tap_cache:
        ...
        collator = DataCollatorACMLMTapCached(tok, frames_per_sec=args.frames_per_sec,
                                              ...
                                              normalize_fn=NORMALIZERS[args.lang])
        eval_collator = DataCollatorACMLMTapCached(tok, frames_per_sec=args.frames_per_sec,
                                                   eos_repeat=args.eos_repeat, fixed_p=1.0,
                                                   normalize_fn=NORMALIZERS[args.lang])
    elif args.acmlm:
        # indic-nar: live-audio A-CMLM collator with the --lang normalizer (hi: NFC + Devanagari only;
        # kn: NFC + Kannada + lowercase Latin), so targets match the vocab_dir built with the same fn.
        # data2vec eats do_normalize'd raw wav (== fairseq layer_norm).
        collator = build_collator(tok, frames_per_sec=args.frames_per_sec,
                                  eos_repeat=args.eos_repeat, normalize_fn=NORMALIZERS[args.lang])
        # eval uses a DETERMINISTIC full-content mask (p=1.0) -> one-shot reconstruction proxy
        eval_collator = build_collator(tok, frames_per_sec=args.frames_per_sec,
                                       eos_repeat=args.eos_repeat, fixed_p=1.0,
                                       normalize_fn=NORMALIZERS[args.lang])
    else:
    ...
    if args.acmlm:
        # indic-nar: A-CMLM head at data2vec width (1024) on a FROZEN data2vec-aqc encoder.
        # args.model_checkpoint = the data2vec-aqc .pt (SSL or CTC-finetuned; loader drops CTC head).
        model = build_d2v_acmlm(tok, d2v_ckpt=args.model_checkpoint, encoder_dim=1024,
                                num_sa_layers=args.num_sa_layers, use_sinusoidal_pe=args.pe,
                                device=dev, mask_prob=args.mask_prob, mask_prob_max=args.mask_prob_max,
                                mask_length=args.mask_length, pos_mode=args.pos_mode,
                                tgt_layer=args.encoder_tgt_layer)
    else:
    ...
    # CACHED-tap mode: the frozen encoder is NEVER called in forward (the tap is read from the
    # memmap and passed as tap=...; see FillerHubertACMLM.forward + eval's encode_from_tap). So it
    # is dead weight on the GPU -> drop it to reclaim ~1.2GB VRAM (data2vec-aqc LARGE, fp32).
    if args.tap_cache and args.acmlm:
        model.hubert = None
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
        if is_main:
            print("[tap_cache] freed frozen encoder from GPU (forward reads cached taps only)", flush=True)
    ...
    if args.lr_schedule == "exponential":
        ...
        # flat LR = --lr for every step (no warmup, no decay). Used to CONTINUE a finished run at its
        # final LR without the jump the exponential schedule makes when total_steps changes on resume.
        from torch.optim.lr_scheduler import LambdaLR
        sched = LambdaLR(opt, lambda _step: 1.0)
    else:
        from transformers import (get_linear_schedule_with_warmup,   # lazy: avoids peft import on the
                                  get_cosine_schedule_with_warmup)    # exponential path (see top note)
    ...
    if args.resume and os.path.exists(args.resume):
        ...
        if args.lr_schedule == "constant":
            # the loaded scheduler state carries the OLD run's base LR (e.g. 2e-4); pin it to --lr so the
            # continued run really trains at exactly --lr (Adam moments are kept from the checkpoint)
            sched.base_lrs = [args.lr for _ in opt.param_groups]
            for _g in opt.param_groups:
                _g["lr"] = args.lr
            sched._last_lr = [args.lr for _ in opt.param_groups]
            if is_main:
                print(f"[resume] constant LR pinned to {args.lr:.2e}", flush=True)
    ...
    if args.wandb and is_main:
        ...
        # RESUME -> continue the SAME wandb run: prefer an explicit WANDB_RESUME_ID (e.g. when out_dir
        # is a different folder than the one the run was created in), else fall back to the id saved in
        # this out_dir's wandb_run_id.txt. Plain resume="allow" by id (NOT the "rewind" feature, which is
        # a paid/private-preview W&B feature and is not enabled on this account) -> W&B just APPENDS new
        # steps to the existing run; it will silently drop any step <= the run's last logged step, so
        # steps already logged (e.g. from an earlier bad-LR attempt) stay as-is in the history.
        _id_file = os.path.join(args.out_dir, "wandb_run_id.txt")
        _resume_id = os.environ.get("WANDB_RESUME_ID", "")
        if not _resume_id and args.resume and os.path.exists(_id_file):
            _resume_id = open(_id_file).read().split()[1]
        _resume_kw = dict(id=_resume_id, resume="allow") if _resume_id else {}
        if _resume_id:
            print(f"[wandb] resuming run id {_resume_id}", flush=True)
        _wandb_common = dict(
            project=os.environ.get("WANDB_PROJECT", "filler_asr"),   # run_kn.sh sets these
            name=os.environ.get("WANDB_NAME") or f"full960_sa{args.num_sa_layers}_pe_lmhead_{'PE' if args.pe else 'noPE'}{mask_tag}{exp_tag}_{os.path.basename(args.model_checkpoint)}",
        ...
        _primary = wandb.init(**_resume_kw, reinit="create_new", **_wandb_common)
        print("wandb run (resumed):", _primary.url, flush=True)
        with open(os.path.join(args.out_dir, "wandb_run_id.txt"), "w") as _f:     # decode_kn_testsets.py
            _f.write(f"{_primary.project} {_primary.id}\n")                      # attaches test graphs here

        # WANDB_MIRROR_NEW=1 -> ALSO log to a brand-new run (no history collision), so steps that the
        # resumed/primary run silently drops (any step <= its last-logged step, e.g. after a resume
        # from an earlier point) are still visible live, immediately, in a second chart. Both runs get
        # the exact same data; the primary keeps the durable id everything else (wandb_run_id.txt,
        # decode --wandb attach) points at.
        _secondary = None
        if os.environ.get("WANDB_MIRROR_NEW", "") == "1":
            _secondary = wandb.init(reinit="create_new",
                                    project=_wandb_common["project"],
                                    name=(os.environ.get("WANDB_MIRROR_NAME")
                                          or f"{_wandb_common['name']}_live_from_step{step if 'step' in dir() else 0}"),
                                    config=_wandb_common["config"])
            print("wandb run (live mirror):", _secondary.url, flush=True)

        class _DualRun:
            """Same .log()/.finish() call sites work unchanged; mirrors every write to both runs.
            wandb Media objects (Table/Image/...) are bound to the ONE run they are first logged to
            ("We don't support referring to media files across runs") -> such keys are logged to the
            primary run only; every plain scalar/number is mirrored to both."""
            def __init__(self, primary, secondary):
                self._runs = [r for r in (primary, secondary) if r is not None]
                self.url, self.id, self.project = primary.url, primary.id, primary.project
            def log(self, data, step=None):
                import wandb as _wandb
                media_keys = {k for k, v in data.items() if isinstance(v, _wandb.data_types.WBValue)}
                for r in self._runs:
                    r.log(data if not media_keys or r is self._runs[0] else
                         {k: v for k, v in data.items() if k not in media_keys}, step=step)
            def finish(self):
                for r in self._runs:
                    r.finish()
        run = _DualRun(_primary, _secondary)
    if is_main:
    ...
    model.train()
    if is_main and raw_model.hubert is not None:                       # change #1 audit: encoder must be eval + frozen
        _hg = sum(p.requires_grad for p in raw_model.hubert.parameters())  # (skipped in tap-cache mode: hubert=None)
